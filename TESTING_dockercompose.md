Docker Compose Drupal derived - php8, nginx, mariadb
====================================================
...
docker run --rm --net drupal-derived_default amazeeio/dockerize dockerize -wait tcp://mariadb:3306 -timeout 1m
```
...
docker ps --filter label=com.docker.compose.project=drupal-derived | grep Up | grep drupal-derived-nginx-1
docker ps --filter label=com.docker.compose.project=drupal-derived | grep Up | grep drupal-derived-mariadb-1
docker ps --filter label=com.docker.compose.project=drupal-derived | grep Up | grep drupal-derived-php-1
docker ps --filter label=com.docker.compose.project=drupal-derived | grep Up | grep drupal-derived-cli-1

# Should ssh against the cli container by default
docker compose exec -T cli bash -c "env | grep LAGOON=" | grep cli-drupal

# Should have the correct environment set
docker compose exec -T cli bash -c "env" | grep LAGOON_ROUTE | grep drupal-derived.docker.amazee.io
